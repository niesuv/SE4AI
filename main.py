...
async def predict_disease(
    ...
    if not session:
        ...
        raise HTTPException(status_code=503, detail="Model is not available. Service might be starting or encountered an error.")

    # Validate file type
    if not file.content_type or not file.content_type.startswith("image/"):
        print(f"Invalid file type received: {file.content_type}")
        raise HTTPException(status_code=400, detail=f"Invalid file type. Please upload an image (received: {file.content_type}).")
